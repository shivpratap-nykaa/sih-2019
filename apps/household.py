...
layout = html.Div(
    [
        ...
        dcc.Dropdown(
            id="house-dropdown",
            options=[
                {"label": i, "value": labelIds[idx]} for (idx, i) in enumerate(labels)
            ],
            value=labelIds[-1],
            style={"margin-bottom": "20px"},
        ),
        dcc.Graph(id="household-bar-graph", style={"padding-top": "20px"}),
        generate_table(data),
    ],
    className="container",
)
...
@app.callback(
    Output("household-bar-graph", "figure"), [Input("house-dropdown", "value")]
)
def update_graph(selected_dropdown_value):
    ...
    graphs = [
        go.Bar(x=year_list, y=array, name=header_set[idx])
        for (idx, array) in enumerate(arrays)
    ]

    return {"data": graphs, "layout": {"title": "test"}}
