...
layout = html.Div(
    [
        html.H1("NVA Time Series"),
        dcc.Dropdown(
            id="nv-my-dropdown",
            options=[
                {"label": category, "value": labelIds[idx]}
                for (idx, category) in enumerate(labels)
            ],
            value=labelIds[-1],
            style={"margin-bottom": "20px"},
        ),
        dcc.Graph(id="nv-time-series", style={"padding-top": "20px"}),
        generate_table(data),
    ],
    className="container",
)
...
@app.callback(Output("nv-time-series", "figure"), [Input("nv-my-dropdown", "value")])
def update_graph(selected_dropdown_value):
    ...
    return {
        "data": [
            go.Bar(x=year_list[:mid], y=row[:mid], name="Current Price"),
            go.Bar(x=year_list[mid:], y=row[mid:], name="Constant Price"),
        ],
        "layout": {"title": data.iloc[index][-1]},
    }
